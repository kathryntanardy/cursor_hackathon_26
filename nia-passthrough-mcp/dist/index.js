...
const NIA_TOOL = "lookup_codebase_context";
...
const session = {
    ...
    total_saved_ms: 0,
    last_5: [],
    prev_cache_size: null,
};
function truncateQuery(q, max = 200) {
    return q.length <= max ? q : q.slice(0, max);
}
function broadcastWss(wss, ev) {
...
function extractTextBlocks(result) {
    ...
    const parts = blocks.filter((b) => {
        return !!b && typeof b === "object" && "type" in b && b.type === "text" && typeof b.text === "string";
    });
    return parts.map((b) => b.text).join("\n");
}
function parseNiaPlaintextResponse(raw) {
    const sourcesMarker = "\n\nSources:\n";
...
function stripLeadingContextBanner(body, user_query) {
...
async function forwardLookupFromNia(niaClient, user_query) {
    const niaTimeoutMs = Number.parseInt(process.env.NIA_TOOL_TIMEOUT_MS ?? "300000", 10);
    const resUnknown = (await niaClient.callTool({
        name: NIA_TOOL,
        arguments: { user_query },
    }, undefined, { timeout: niaTimeoutMs }));
    ...
    if ("structuredContent" in resUnknown && resUnknown.structuredContent && typeof resUnknown.structuredContent === "object") {
        const sc = resUnknown.structuredContent;
        if (typeof sc.content === "string" && Array.isArray(sc.sources)) {
            return { content: sc.content, sources: sc.sources.filter((x) => typeof x === "string") };
        }
    }
    text = stripLeadingContextBanner(text, user_query);
    return parseNiaPlaintextResponse(text);
}
async function postLookup(cacheBase, query) {
    ...
    try {
        ...
        const similarity = raw.similarity === null || raw.similarity === undefined
            ? null
        ...
        return {
            ...
            cached_query: typeof raw.cached_query === "string" ? raw.cached_query : null,
        };
    }
    catch {
        return null;
...
async function postInsert(cacheBase, query, response) {
...
async function avgNiaLatencyMs(niaClient) {
    const q = process.env.NIA_WARMUP_QUERY ??
...
function similarityForQueryComplete(status, similarity) {
    if (status === "EXACT_HIT" || status === "MISS") {
        return null;
    }
    return similarity;
}
async function fetchFromNiaWithFailOpen(niaClient, user_query) {
    const tNia = Date.now();
    try {
        ...
        console.error("[cache-wrapped-nia] Nia error — fail-open MISS-style payload; /insert skipped (avoid poisoning cache).", msg);
        return {
...
async function main() {
    ...
    server.registerTool(NIA_TOOL, {
        ...
        console.error(`[timing] query_id=${query_id} cache_lookup_ms=${lookupMs} nia_roundtrip_ms=${niaRoundtripMs ?? "NA"} wall_total_ms=${latency_ms} status=${status} nia_fail_open=${niaFailOpen}`);
