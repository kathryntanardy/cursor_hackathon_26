...
| `NIA_MCP_REMOTE_URL` | No | **Remote MCP only** (`mcp:install:remote`): HTTP URL for Nia MCP. Default `https://apigcp.trynia.ai/mcp`. |
...
### Option A — **Remote Nia** (TryNia hosted MCP, recommended by [TryNia docs](https://docs.trynia.ai/integrations/installation/mcp#cursor))

No local gateway, no `npm run build` for Person 1. Your API key is written into **`mcp.json`** (keep that file private):

```bash
npm run mcp:install:remote
```

This adds server **`nia`** with `url` + `Authorization: Bearer …`, and **removes** **`cache-wrapped-nia`** so you are not running two Nia setups. To go back to the cache gateway:

```bash
npm run mcp:install
```

Optional in **`.env`**: `NIA_MCP_REMOTE_URL` (default `https://apigcp.trynia.ai/mcp`).

**Restart Cursor** after any `mcp.json` change.

### Option B — **cache-wrapped-nia** (local gateway + cache + WebSocket per CONTRACTS)

Writes **`cache-wrapped-nia`** into your user **`~/.cursor/mcp.json`**. The JSON points at **`scripts/run-gateway.mjs`**, which loads **`.env`** and runs **`dist/index.js`** — so the Nia secret stays out of **`mcp.json`**.
...
- In Cursor, enable **`nia`** (remote) or **`cache-wrapped-nia`** (local gateway) and use the Nia codebase tool (e.g. **`lookup_codebase_context`**) with `user_query`.
...
| `MCP error -32000: Connection closed` on startup | Run **`npm install`** in **`nia-passthrough-mcp`** (pins **`nia-codebase-mcp`** locally; the gateway spawns it with the **same Node** as Cursor, no npx). If it persists: **`NIA_FORCE_NPX=1`** and fix **`npx`** on PATH, or run **`node scripts/probe-nia-spawn.mjs`**. **Unix pipx** default: ensure **`pipx run nia-mcp-server`** works. |
